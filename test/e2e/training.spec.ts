...
test('training', async ({page}) => {
  ...
  await page
    .getByRole('link', {
      name: 'Training Learn how to maximize our platform to get the most out of it.'
    })
    .click();
  await expect(page).toHaveURL('/training');
  await expect(page.getByRole('heading', {name: 'Our Training'})).toBeVisible();
  await expect(
    page.getByRole('link', {name: 'SAF Training Site - GitHub'})
  ).toBeVisible();
  await expect(
    page.getByRole('link', {name: 'SAF Training Site - Netlify'})
  ).toBeVisible();
  await expect(
    page.getByRole('heading', {name: 'Beginner Class'})
  ).toBeVisible();
  await expect(
    page.getByRole('heading', {name: 'Advanced Class'})
  ).toBeVisible();
  await expect(
    page
