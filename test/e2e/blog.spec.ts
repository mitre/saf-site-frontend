...
test('Blog', async ({ page }) => {
  ...
  await expect(page.getByRole('img', { name: 'Validation Lib Icon-Dark.svg' })).toBeVisible();
  await expect(page.getByRole('img', { name: 'wtm.png' })).toBeVisible();

  // Check video load
  await expect(page.frameLocator('internal:attr=[title="YouTube video player"i]').locator('.ytp-cued-thumbnail-overlay-image')).toBeVisible();
