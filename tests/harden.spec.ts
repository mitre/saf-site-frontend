...
//   await expect(page).toHaveURL('http://localhost:3000/toolkit/harden')
...
//   await page.goto('http://localhost:3000/toolkit/harden');  
...
test('test', async ({ page }) => {
  ...
  await page.getByRole('cell', { name: 'AWS CIS Benchmark | v1.1.0' }).click();
  await page.getByRole('button', { name: 'View Details , AWS CIS Benchmark' }).click();
  await expect(page).toHaveURL('http://localhost:3000/toolkit/modal/aws-cis-benchmark?id=83')
  await page.locator('dl > div > div').first().click();
  await page.goto('http://localhost:3000/toolkit/harden');
  await page.getByRole('cell', { name: 'Apache CIS Benchmark | v2.0.0' }).click();
  await page.getByRole('button', { name: 'View Details , Apache CIS Benchmark' }).click();
  await expect(page).toHaveURL('http://localhost:3000/toolkit/modal/apache-cis-benchmark?id=72')
  await page.locator('dl > div > div').first().click();
  await page.goto('http://localhost:3000/toolkit/harden');
  ...
  await expect(page).toHaveURL('http://localhost:3000/toolkit/modal/cisco-ios-xe-ndmrtr-stig?id=58')
  await page.locator('dl > div > div').first().click();
});