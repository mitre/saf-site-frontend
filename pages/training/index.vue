          ...
          <div class="flex-1 mx-auto">
            <div id="Content Area" class="relative max-w-2xl mx-auto sm:px-6 py-4 px-4  sm:py-4 lg:max-w-6xl lg:px-8">
              <div id="Overview Section" class="sm:pl-4">
                <h1 class="block text-3xl leading-8 font-extrabold tracking-tight text-header  sm:text-4xl">
                  Our Training</h1>
                <p class="mt-6 text-base "> The MITRE SAF© team offers training classes. Dates
                  and sign-up
                  links
                ...
                <div class="flex gap-8">
                  <a href="https://mitre.github.io/saf-training/" class="hover:underline">
                    <p class="mt-4 text-nav-active  after:content-['_↗']"> SAF Training
                      Site - GitHub</p>
                  </a>
              ...
              <div id="Schedule" class="relative pointer-events-auto order-2 pt-8 pr-6">
                <div>
                  <h2 class="text-xl font-semibold text-base  sm:pl-3 pt-6 ">Upcoming Classes
                  </h2>
                    ...
                    <ol class="mt-4 text-sm leading-6 lg:col-span-7 xl:col-span-8">
                      <li v-for="schedule in schedules" :key="schedule.index" class="relative flex space-x-6 py-6 ">
                        <div class="flex-0 pl-4">
                          <h3 class="font-semibold text-base ">{{ schedule.title }}</h3>
                        ...
                        <div v-if="schedule.link != null" class="right-2 top-0 pt-3 flex-end">
                          <a type="button" :href="schedule.link" target="_blank"
                            class="inline-flex items-center rounded-full border border-transparent bg-button-accent  text-button-text  hover:bg-opacity-80 sm:px-3 sm:py-1.5 px-3 py-1.5 text-xs text-center font-medium shadow-sm ring-1 ring-accent ">Sign
                            Up</a>
                        </div>
                        <div v-else class="right-2 top-0 pt-3 flex-end">
                          <a type="button" aria-disabled="true"
                            class="inline-flex items-center rounded-full border border-transparent bg-button-disabled  text-button-disabled-text  sm:px-3 sm:py-1.5 px-3 py-1.5 text-xs text-center font-medium shadow-sm">
                            Coming Soon!
                          </a>
                        </div>
                      </li>
                  ...
                  <div v-else class="relative max-w-sm mt-6 pl-2 text-base ">
                    <p> Sorry there are no upcoming classes at this time. If you would like to host a class please
                      ...
                      <a href="mailto:saf@groups.mitre.org"
                        class="hover:underline text-nav-active ">saf@groups.mitre.org</a>
                    </p>
                  </div>
                </div>
              </div>
              <div id="Embeded Videos" class="flex-1 order-1 pt-9">
                <h2 class="block text-3xl leading-8 font-bold tracking-tight text-base  sm:text-3xl pt-4 sm:pl-4">
                  Classes </h2>
                <hr class="mt-4 w-full" />
                <div v-if="courses.length">
                  <div v-for="(course, index) in courses" :key="course.index">
                    <div class="sm:pl-4 mt-7 max-w-xl">
                      <p class="text-2xl font-semibold text-base ">
                        {{ course.name }}
                      </p>
                      <div
                        class="max-w-3xl mt-8 mb-8 leading-8 text-left prose prose-md lg:prose-lg dark:prose-invert text-base  prose-li:text-start prose-code:text-start"
                ...
                <div v-else>
                  <p class="relative max-w-sm mt-6 pl-2 text-base "> Classes are being updated.
                    Check back
                    later!
                  </p>
                </div>
