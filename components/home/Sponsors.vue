<template>
  <div class="px-6">
    <div class=" mx-auto py-6 px-6 lg:py-12 lg:max-w-7xl md:max-w-2xl max-w-md">
      <div
        class="overflow-hidden rounded-lg bg-neutral-1  mx-auto grid w-full max-w-3xl lg:max-w-none grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-7 items-center sm:gap-px sm:divide-y-0 gap-y-12 gap-x-10 sm:gap-x-6 sm:gap-y-14 lg:mx-0 lg:gap-x-6 ">
          ...
          <div class="mt-6 sm:mt-6">
            <h3 class="text-base font-semibold leading-6">
              <NuxtLink :to="entry.link" class="focus:outline-none">
                ...
                <p class="text-base  mt-6 text-center hidden md:block">{{ entry.nameLong }}</p>
                <p class="text-base  mt-6 text-center md:hidden">{{ entry.name }}</p>
              </NuxtLink>
            </h3>
          </div>
          <span
            class="pointer-events-none absolute right-0 top-0 sm:right-0 sm:top-0 text-base  group-hover:text-nav-active "
            aria-hidden="true">
            <DiagonalArrow />
          </span>
        </div>
