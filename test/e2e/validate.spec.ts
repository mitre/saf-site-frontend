...
test('Validation', async ({ page }) => {
  ...
  await expect(page).toHaveURL('/framework/guidance/gke-cis-benchmark?id=6')
  ...
  await page.goto('/framework/validate');

  await page.getByRole('button', { name: 'View Details , Ubuntu 16.04 STIG' }).click();  
  await expect(page).toHaveURL('/framework/guidance/ubuntu-1604-stig?id=27')
  await expect(page.getByRole('heading', { level: 3, name: 'Guidance Overview' })).toBeVisible();
  ...
  await expect(page).toHaveURL('/framework/guidance/windows-2012-stig?id=37')
  await expect(page.getByRole('heading', { level: 3, name: 'Guidance Overview' })).toBeVisible();
