<template>
  <nav aria-labelledby="on-this-page-title" class="ml-5 w-56">
    <div v-if="props.tableOfContents.length > 0">
      <h2 id="on-this-page-title" class="font-bold text-sm text-base ">
        On this page
      </h2>
          ...
          <li key={{heading.title}}>
            <h3>
              <a :href="`#${slugify(heading.title)}`"
                :class="props.currentHeading == slugify(heading.title) ? 'text-nav-active ' : `font-normal text-nav-base hover:text-nav-hover  `">
                {{ heading.title }}
              </a>
            </h3>
                ...
                <div v-for="subtitle in heading.subtitles">
                  <li key={{subtitle}}>
                    <a :href="`#${slugify(subtitle)}`"
                      :class="props.currentHeading == slugify(subtitle) ? 'text-nav-active ' : 'hover:text-nav-hover '">
                      {{ subtitle }}
                    </a>
                  </li>
                </div>
