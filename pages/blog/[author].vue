                ...
                <div class="mx-auto px-4 sm:px-6 lg:px-8">
                  <div class="mt-2 sm:-mt-16 sm:flex sm:items-end sm:space-x-5">
                    <div v-if="author.photo" class="flex">
                      <img class="ring-base h-24 w-24 rounded-full p-2 ring-4 sm:h-32 sm:w-32"
                        :src="author.photo.url ?? ''" :alt="author.photo.name ?? ''" />
                    </div>
              ...
              <div class="mx-auto mt-6 px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 gap-y-8 sm:grid-cols-2">
                  ...
                  <div class="sm:col-span-2">
                    ...
                    <div class="prose mt-1 max-w-prose space-y-5 text-sm text-foreground" v-html="author.description" />
                  </div>
                </div>
                <div id="Social Medias" class="flex gap-2 pb-5 pt-5">
                  <a v-if="socialMediaLinks.displayEmail !== null" :href="'mailto:' + socialMediaLinks.displayEmail">
                    <span class="sr-only">Mail Link</span>
                    <MailIcon class="h-9 w-9 text-base" />
                  </a>
                  <a v-if="socialMediaLinks.twitter" :href="socialMediaLinks.twitter">
                    <img src="~/assets/logos/TwitterLogo.png" class="h-auto max-h-8 max-w-full" alt="Twitter Logo" />
                  </a>
                  <a v-if="socialMediaLinks.facebook" :href="socialMediaLinks.facebook">
                    <img src="~/assets/logos/FacebookLogo.png" class="h-auto max-h-8 max-w-full" alt="Facebook Logo" />
                  </a>
                  <a v-if="socialMediaLinks.instagram" :href="socialMediaLinks.instagram">
                    <img src="~/assets/logos/InstagramLogo.png" class="h-auto max-h-8 max-w-full" alt="Instagram Logo" />
                  </a>
                  <a v-if="socialMediaLinks.github" :href="socialMediaLinks.github" class="dark:hidden">
                    <img src="@/assets/logos/GitHubLogoBlack.svg" class="h-auto max-h-8 max-w-full" alt="GitHub Logo" />
                  </a>
                  <a v-if="socialMediaLinks.github" :href="socialMediaLinks.github" class="hidden dark:flex">
                    <img src="@/assets/logos/GitHubLogoWhite.svg" class="h-auto max-h-8 max-w-full" alt="GitHub Logo" />
                  </a>
                  <a v-if="socialMediaLinks.linkedin" :href="socialMediaLinks.linkedin">
                    <img src="~/assets/logos/LinkedInLogo.png" class="h-auto max-h-8 max-w-full" alt="LinkedIn Logo" />
                  </a>
  ...
  author.value = await useAsyncData('getBlogAuthor', () =>
    ...
    return {
      ...
      fields: {
        ...
        company:
          authorObj?.attributes?.partner?.data?.attributes?.name.replace(
            '_',
            ' '
          )
      },
